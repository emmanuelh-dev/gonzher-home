...
export default function ServiceCard({ service, children }: Props) {
  ...
  return (
    <div className='mb-12 p-8 bg-primary rounded-2xl text-white flex flex-col items-center justify-center'>
      ...
      <p className={`overflow-hidden transition-all duration-500 ${open ? 'h-72' : 'h-0'}`}>{descripcion}</p>
      <button title={`Ver mas ${titulo}`} className="py-2 font-medium text-base text-white/80 hover:text-white" onClick={() => setOpen(!open)}>
