...
const MobileNav = () => {
            ...
            {section.sections &&
              section.sections.map((subSection: any) => (
                <div key={subSection.title}>
                  <h3 className="ml-6 font-semibold text-neutral-600">
                    {subSection.title}
                  </h3>
    ...
    if (section.type === "links") {
      return section.items.map((link: any) => (
        <div key={link.href} className="border-b border-gray-200">
          <Link
            href={link.href}
            className="block px-6 py-4 font-medium text-gray-800 hover:bg-gray-50"
            onClick={onToggleNav}
          >
            {link.title}
          </Link>
  ...
  return (
    <div className="sm:hidden">
      ...
      <div
        className={`fixed left-0 top-0 z-50 h-full w-full transform bg-white shadow-xl transition-transform duration-300 ease-in-out ${
          navShow ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <div className="font-bold text-gray-800">Menú</div>
          <button
            type="button"
